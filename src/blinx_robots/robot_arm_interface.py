...
class BlxRobotArm(object):
    ...
    def command_receiver(self):
        ...
        with self.communication_strategy.connect() as client:
            while self.thread_work_flag:
                ...
                # todo 分流数据
                logger.debug(f"接收数据: {data.strip()}")
                self.recv_data_buffer.put(data)
    ...
    def set_time_delay(self, delay_time: int):
        ...
        if self.robot_cmd_model == "SEQ":
            ...
            if 0 <= delay_time <= 3000:
                # 获取命令执行结果
                self.task_executor.submit(self.get_command_response, "set_time_delay")
    ...
    def set_robot_arm_init(self) -> bool:
        """机械臂初始化，将机械臂关节角度归零
        ...
        robot_arm_init_result = json.loads(robot_arm_init_status.result()).get('data')
        logger.debug(f"机械臂初始化命令: {robot_arm_init_result}")
        return robot_arm_init_result

    def set_robot_arm_home(self) -> dict:
        """机械臂回零
    ...
    def set_robot_end_tool(self, io: int, status: bool) -> bool:
        """设置机械臂 IO 口状态
        :param io: 机械臂IO口, 0~3
        ...
        self.command_queue.put(command)
        end_tool_status = json.loads(end_tool_status.result()).get('data')
        logger.debug(f"机械臂末端工具状态: {end_tool_status}")
        return end_tool_status
